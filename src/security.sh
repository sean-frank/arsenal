#!/usr/bin/env bash
# security.sh
# checks for a security.txt file for given url(s)
...
for i in "${!args[@]}"; do
    ...
    if [[ "$url" =~ ^https?:// ]]; then
        ...
        sec_url="https://$domain/.well-known/security.txt"
        ...
        if curl -s -IL "$sec_url" | grep -qE "HTTP\/.+200"; then
            ...
            if [ "$FULL" = true ]; then
                ...
                printf '%s\n' "${GREEN}✓${RESET} ${sec_url} ${GREEN}has a security.txt file.${RESET}"
            else
            ...
            if [ "$FULL" = true ]; then
                # Print the full output
                printf '%s\n' "${RED}✗${RESET} ${sec_url} ${RED}does not have a security.txt file.${RESET}"
            else
                # Print the simple output
                printf '%s\n' "${RED}✗${RESET} ${sec_url}"
            fi
