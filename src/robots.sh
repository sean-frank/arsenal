#!/usr/bin/env bash
# security.sh
# Description: checks for a robots.txt file for a domain
...
if [ "$dupes" -gt 0 ]; then
    printf '%s\n\n' "${BG_YELLOW}${BOLD}Duplicate domain(s) found, ${dupes} removed.${RESET}"
fi
...
for i in "${!args[@]}"; do
    ...
    if curl -s -IL "$url" | grep -qE "HTTP\/.+200"; then
        ...
        if [ "$FULL" = true ]; then
            ...
            printf '%s\n' "${GREEN}✓${RESET} ${url} ${GREEN}has a robots.txt file.${RESET}"
        else
        ...
        if [ "$FULL" = true ]; then
            # Print the full output
            printf '%s\n' "${RED}✗${RESET} ${url} ${RED}does not have a robots.txt file.${RESET}"
        else
            # Print the simple output
            printf '%s\n' "${RED}✗${RESET} ${url}"
        fi
