#!/usr/bin/env bash
# ./src/spotter.sh
# Description: check url status, follow redirects, and print the results (NO VISUAL)
...
function display_help() {
    echo "Usage: spotter [OPTIONS]... [URL or FILE]"
    echo "Read URLs from stdin, a file, or direct argument and print their status to stdout"
    echo "If no input or file is provided, spotter drops into a TTY for input value"
    echo " "
    echo "Options:"
    ...
    echo " "
    echo "Examples:"
    echo "  echo 'https://example.com' | spotter"
    echo "  spotter <(echo 'https://example.com')"
    echo "  spotter <<< 'https://example.com'"
    echo "  spotter"
    exit 0
}
...
function display_header() {
    printf '%s\n' "${RED}             _   _           ${RESET}"
    printf '%s\n' "${RED} ___ ___ ___| |_| |_ ___ ___ ${RESET}"
    printf '%s\n' "${RED}|_ -| . | . |  _|  _| -_|  _|${RESET}"
    printf '%s\n' "${RED}|___|  _|___|_| |_| |___|_|  ${RESET}"
    printf '%s\n' "${RED}    |_|                      ${RESET}"
...
for i in "${!args[@]}"; do
    ...
    while [ -n "$url" ]; do
        ...
        response=$(
            curl --insecure --fail --show-error --no-progress-meter \
        ...
        response=$(sed 's/\r$//' <<<"$response")

        http_status=$(grep -E '^HTTP\/.*$' <<<"$response")
        location=$(grep -E '^Location: ' <<<"$response" | cut -d ":" -f 2- | sed -E 's/^[[:space:]]+//g')
        response=$(sed -E '/^(HTTP\/|Location: )/d' <<<"$response")

        http_version=$(cut -d " " -f 1 <<<"$http_status")
        status_code=$(cut -d " " -f 2 <<<"$http_status")
        status_reason=$(cut -d " " -f 3- <<<"$http_status")

        headers_filtered=$(grep -iE "$header_filters" <<<"$response")
        if [ $FULL = true ]; then
            headers_filtered="$response"
        fi
